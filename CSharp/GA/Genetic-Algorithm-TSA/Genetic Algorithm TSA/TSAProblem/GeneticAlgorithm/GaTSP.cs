...
namespace TSAProblem.GeneticAlgorithm
{
    ...
    public class GaTSP
    {
        ...
        private List<int> Mutation(List<int> vector)
        {
            ...
            if (objRandom.NextDouble() > mutationRate) return lstMutated;

            int pos1 = objRandom.Next(0, chromosomeLength);
            int pos2 = pos1;

            while (pos1 == pos2)
            {
                pos2 = objRandom.Next(0, chromosomeLength);
            }
        ...
        private void Crossover(List<int> mum, List<int> dad, out List<int> baby1, out List<int> baby2)
        {
            baby1 = Copy(mum);
            baby2 = Copy(dad);
            if (objRandom.NextDouble() > crossoverRate || AreEqual(mum, dad))
            {
                return;
            }
        ...
        public void Epoch()
        {
            ...
            while (lstNewPop.Count <= populationSize)
            {
        ...
        private void CreateStartingPopulation()
        {
            for (int i = 0; i < populationSize; i++)
            {
                Genome objGenome = new Genome(chromosomeLength,objRandom);
